...
Map2GFF::Map2GFF(const std::string& tlstFP, const std::string& alFP, const std::string& multiFP, const std::string& glstFP, const bool& multi_flag, const int& threads){
            ...
            while (std::getline(multistream,mline)){
                #pragma omp task
                {
                    ...
                    while(std::getline(ss,pretab,',')){
                        ...
                        if(getEntryLength){ // if first entry - get length of the database
                            this->db_entry_length=this->db_entry_length+(end-start);
                        }
                        cur_coords1.push_back(std::make_pair(start,end));
                    }
                    if (getEntryLength){
                        std::cout<<"entry size: "<<this->db_entry_length<<std::endl;
                    }
...
int Map2GFF::merge_cigar(const std::vector<std::pair<int,int>> *cor,bam1_t *al, uint32_t *cur_cigar_full, int n_cigar){
    ...
    int copy3_len = al->l_data - copy1_len - (al->core.n_cigar * 4);
    memcpy(data + copy1_len + copy2_len, bam_get_seq(al), copy3_len);

    al->core.n_cigar = mult_num_cigars;
...
void Map2GFF::convert_coords(const std::string& outFP, const std::string& genome_header){
    ...
    bool doMulti=true; // to look for multimappers or not
                ...
                if (mate_ret_val && paired && pairedly_aligned){
                    if (curAl->core.flag&64){
                        ...
                        if (curReadGroup_tmp.find(tmpKey_pre)==curReadGroup_tmp.end()){
                            curReadGroup_tmp[tmpKey_pre]=new MateRead(bam_dup1(curAl),read_start,cigar_str,al_coords,cur_cigar_full,n_cigar);
                        }
                        else{
                                                    ...
                                                    for(auto cor2:this->exists_al_coord_mate2->second){
                                                        ...
                                                        if(this->exists_geneID_mate1!=this->gene_coords.end() &&
                                                           ...
                                                           this->exists_geneID_mate1->second==this->exists_geneID_mate2->second){ // both identified, and geneIDs match
                                                            ...
                                                            add_multimapper_pair(cor1,cor2,curAl,mr->al,cur_cigar_full,mr->cigar_full,n_cigar,mr->n_cigar);
                                                        }
                                                    }                                            
                                                }
                                            }
                                                    ...
                                                    for(auto cor1:this->exists_al_coord_mate1->second){
                                                        ...
                                                        if(this->exists_geneID_mate2!=this->gene_coords.end() &&
                                                           ...
                                                           this->exists_geneID_mate2->second==this->exists_geneID_mate1->second){ // both identified, and geneIDs match
                                                            ...
                                                            add_multimapper_pair(cor2,cor1,mr->al,curAl,mr->cigar_full,cur_cigar_full,mr->n_cigar,n_cigar);
                                                        }
                                                    }                                            
                                                }
                                            }
