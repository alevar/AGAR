...
int Comparator::write_remaining_trans_pair(samFile* al,bam_hdr_t* al_hdr,bam1_t* curAl,bam1_t* mate,bool both){
    ...
    while(true) { // only perfom if unaligned flag is set to true
        ...
        if(std::strcmp(bam_get_qname(curAl),readname.c_str()) == 0){
            ...
            add_T2G_aux(curAl); // add T2G tag
            if(check_mate){
                process_status(mate,curAl,check_mate,al_hdr);
            }
            else{
...
void Comparator::_process_disc(bam1_t *curAl,bam1_t* mate,bam_hdr_t* al_hdr){
...
void Comparator::process_disc(bam1_t *curAl,bam1_t *mate,bam_hdr_t* al_hdr) {
    _process_disc(curAl,mate,al_hdr);
}

void Comparator::_process_pair(bam1_t *curAl,bam1_t* mate,bam_hdr_t* al_hdr){
    size_t cigar_hash,mate_cigar_hash;
...
void Comparator::process_pair(bam1_t *curAl,bam1_t *mate,bam_hdr_t* al_hdr) {
    _process_pair(curAl,mate,al_hdr);
}
...
void Comparator::process_single(bam1_t *curAl,bam_hdr_t* al_hdr){
    Position cur_pos;
    int cigars[MAX_CIGARS];
    int num_cigars=0;
...
void Comparator::process_single_mate(bam1_t *aligned, bam1_t *unaligned,bam_hdr_t* al_hdr){
    process_single(aligned,al_hdr);
    aligned->core.mpos = 0;
    aligned->core.mtid = -1;
    finish_read(aligned);
...
int Comparator::evaluate_multimappers(bam1_t* curAl,Position& cur_pos,int cigars[MAX_CIGARS],int &num_cigars){
    int unique;
...
void Comparator::process_status(bam1_t *mate,bam1_t *curAl,bool &check_mate,bam_hdr_t* al_hdr){
    if(std::strcmp(bam_get_qname(curAl),bam_get_qname(mate)) == 0){ // is a pair
        ...
        switch(status){
            case STATUS::CONC:
                this->process_pair(curAl,mate,al_hdr);
                break;
            case STATUS::DISC:
                this->process_disc(curAl,mate,al_hdr);
                break;
            case STATUS::ALIGNED_MIXED_1:
                this->process_single_mate(mate,curAl,al_hdr);
                break;
            case STATUS::ALIGNED_MIXED_2:
                this->process_single_mate(curAl,mate,al_hdr);
                break;
...
//                break;
        ...
        switch(status){
            case STATUS::ALIGNED_SINGLE:
                this->process_single(mate,al_hdr);
                break;
