...
public:
    ...
    void print(){
        std::cout<<this->locid<<":"<<this->chr<<"@"<<this->elen<<this->strand<<this->start<<" "<<this->end<<std::endl;
    }

    std::string get_string(){
        std::stringstream res;
        res<<this->locid<<":"<<this->chr<<"@"<<this->elen<<this->strand<<this->start<<" "<<this->end;
        return res.str();
    }
...
private:
    ...
    uint32_t elen = 1;
    uint32_t start=MAX_INT,end=0,chr=0;
    uint8_t strand;
    uint32_t locid=0;
    float abundance = 0.0;
    bool empty = true;
...
bool moves_eq(const std::vector<uint32_t>& m1,const std::vector<uint32_t>& m2) { // m1 must be smaller or equal to m2
    for(int i=0;i<m1.size();i++){
        if(m1[i] != m2[i]){
            if(i == m1.size()-1 && m1[i] <= m2[i]){ // last move is a subset
                return true;
            }
            return false;
        }
...
public:
    ...
    int get_nh(Position& pos){
        this->ltf = this->lookup_table.find(pos);
        if(this->ltf == this->lookup_table.end()){ // position is not a multimapper
            return -1;
        }
        else{
            std::unordered_set<Position,PosHash_noLoc_noStrand,PosEq_noLoc_noStrand> poss;
            // need to evaluate the multimapping block to see if there exist different strands at the same position
            ...
            while(this->ii->second){ // iterate until the end of the block
                poss.insert(this->ii->first);
                this->ii++; // step to the next position
            }
            // same here for the last iteration
            poss.insert(this->ii->first);
            return poss.size();
        }
    }

    int get_nh_pair(Position& pos, Position& pos_mate){
        this->ltf = this->lookup_table.find(pos);
        if(this->ltf == this->lookup_table.end()){ // position is not a multimapper
            return -1;
        }
    ...
    int get_multi_xs(Position& pos){
//        std::cout<<pos.get_strg()<<std::endl;
        this->ltf = this->lookup_table.find(pos);
        if(this->ltf == this->lookup_table.end()){ // position is not a multimapper
            return -1;
        }
            ...
            int num_found = 0;
            bool neg_strand = false, pos_strand = false;
            // need to evaluate the multimapping block to see if there exist different strands at the same position
            this->ii = this->index.begin()+this->ltf->second; // get iterator to the start of a multimapping block in the array
            while(this->ii->second){ // iterate until the end of the block
                if(this->ii->first.chr == pos.chr &&
                   this->ii->first.start == pos.start){
                    num_found++;
                    if(this->ii->first.strand == '+'){
                        pos_strand=true;
                    }
                    else{
                        neg_strand=true;
